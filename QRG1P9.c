...
for(i=0;i<mrows;i++)
			...
			A[i+j*mrows]=rand()/(double) RAND_MAX;
...
//printf("\n\n\n");
	...
	if(rank==0){
		...
		for(ic=q-2;ic>0;ic--){
			...
			MPI_Recv(&r[ncols*cols*ic+j*ncols],1,columntype,ic,tagr,MPI_COMM_WORLD, &stat);}
...
for(i=0;i<cols;i++){
			...
			MPI_Send(&r[ncols*cols*rank+i*ncols],1,columntype,0,tags,MPI_COMM_WORLD);}
